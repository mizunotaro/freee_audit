...
import type { BoxToken, BoxTokenResponse } from './types'

const TOKEN_BUFFER_SECONDS = 5 * 60

export async function saveToken(companyId: string, tokenResponse: BoxTokenResponse): Promise<void> {
...
export async function getToken(companyId: string): Promise<BoxToken | null> {
  const tokenRecord = await prisma.boxToken.findUnique({
    where: { companyId },
  })
  ...
  try {
    ...
    console.error('Failed to decrypt Box token:', error)
    return null
  }
}

export async function deleteToken(companyId: string): Promise<void> {
  await prisma.boxToken.delete({
    where: { companyId },
  })
}
...
export function parseTokenResponse(response: unknown): BoxTokenResponse | null {
