...
function generateTemplateContent(section: string, companyName: string, fiscalYear: number): string {
  ...
  const templates: Record<string, string> = {
    businessOverview: `${safeCompanyName}は、${safeFiscalYear}年度において[主な事業内容]に注力し、市場での地位を強化いたしました。
...
${safeFiscalYear}年度の[業界]は、[市場動向]の影響を受け、[成長率や市場規模]となりました。
...
[災害や緊急時の対応計画]`,
    ...
    financialHighlights: `【業績ハイライト】（${safeFiscalYear}年度）
...
${safeFiscalYear}年度の研究開発費は[金額]（売上高比[%]）であり、前年度比[増減率]%となりました。
