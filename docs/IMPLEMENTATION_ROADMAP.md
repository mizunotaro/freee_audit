...
## Phase 2.5: 予実管理（Budget Management）（2-3週間）

### 2.5.1 予算策定支援
- [x] BudgetPlan, BudgetPlanItem Prismaモデル（✅ 実装済み）
- [x] 予算計画CRUD API（✅ 実装済み）
- [ ] Excelフォーマット提供（予算策定用テンプレート）
  - Claude Code/Coworkが読み書きしやすい構造化フォーマット
  - 勘定科目×月の2次元マトリックス
  - セクション: 売上、売上原価、販管費、営業外収益/費用
- [ ] AI予算策定支援プロンプト（事業計画・人員計画との整合性チェック）
  ```
  以下の情報を基に、{fiscalYear}年度の月次予算案を作成してください:
  - 事業計画: {businessPlanSummary}
  - 人員計画: {headcountPlan}
  - 前年実績: {previousYearActuals}
  - KPI目標: {kpiTargets}
  出力はJSON形式で、勘定科目×月のマトリックスとしてください。
  ```
- [ ] 予算ファイル読込時のLLM整合性チェック
  - 事業計画との乖離チェック
  - 人員計画と人件費の整合性
  - 前年比の妥当性分析
  - 季節性パターンの検証
...
### 2.5.2 予実差異管理
- [x] BudgetVariance Prismaモデル（✅ 実装済み）
- [x] 差異計算・有意差抽出ロジック（✅ 実装済み、10%閾値）
- [x] 差異理由記録機能（✅ 実装済み、boardReportNote付き）
- [ ] 予実差異ダッシュボードUI
  - 月次推移チャート（予算 vs 実績）
  - 有意差異のハイライト表示
  - ドリルダウン（費目→部門→取引明細）
- [ ] LLM差異分析（予算外れの原因推定、改善提案）
- [ ] 取締役会資料への自動反映
  - 有意差異+理由メモ → 取締役会報告書セクション
  - `getVariancesForBoardReport()` を既存Board Report機能に統合

### 2.5.3 予算と外部データの連携
- [ ] freee仕訳データから実績値の自動集計
- [ ] 事業計画ファイル（Excel/PDF）の読込
- [ ] AIによる予算vs事業計画の整合性レポート生成

---

## Phase 2.6: データバックアップ・エクスポート（1-2週間）

### 2.6.1 データベースバックアップ
- [x] BackupRecord Prismaモデル（✅ 実装済み）
- [x] バックアップ作成サービス（✅ 実装済み、JSON形式）
- [x] バックアップ履歴管理（✅ 実装済み）
- [x] リストア機能（✅ 実装済み）
- [ ] 定期自動バックアップ（node-cronスケジューラー統合）
  - 毎日深夜にフルバックアップ
  - `src/jobs/scheduler.ts` にジョブ追加
- [ ] Box連携バックアップ
  - Box API経由でバックアップファイルを自動アップロード
  - 既存 `src/lib/integrations/box/` の活用
  - 保存先フォルダ設定
  - バックアップローテーション（世代管理）

### 2.6.2 データエクスポート・インポート
- [x] JSONエクスポート（✅ 実装済み）
- [x] CSVエクスポート（✅ 実装済み）
- [ ] Excelエクスポート（既存export/excel.ts活用）
- [ ] データインポート機能
  - JSON/CSVインポート
  - データ整合性チェック付き
  - ドライラン（プレビュー）モード
- [ ] テーブル単位の選択的エクスポート/インポート
...
| 2026-04-15 | Phase 2.5(予実管理), Phase 2.6(バックアップ)追加、16 Prismaモデル+7サービス+8 APIルート実装 | Opus 4.6 |
